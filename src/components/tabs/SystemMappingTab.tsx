...
export const SystemMappingTab: React.FC<SystemMappingTabProps> = ({ data, onDataUpdate, onNavigateToEstimates, projectId, floorSectionMappings = [], systemActivityMappings = [], buildingSectionMappings = [], onBuildingMappingsChanged, importedCostCodes = [], datasetProfile, onProfileOverride, onReanalyzeProfile, onUnappliedChangesUpdate, costHeadActivityOverrides = [], suggestedBuildingMappings = [], codeFormatMode = 'standard', tradePrefix }) => {
  ...
  const handleApplySectionCodes = useCallback(async (_floorMappingsToApply: Record<string, string>, activityMappingsToApply: Record<string, string>, overrideSnapshot?: Array<{ cost_head: string; building_identifier: string | null; use_level_activity: boolean }>) => {
    // Use live snapshot when provided (from Save & Re-apply in Per-Head Activity Overrides).
    // Fall back to saved query state only when called from the standard Re-apply Sections button.
    const effectiveOverrides = overrideSnapshot ?? costHeadActivityOverrides;
    let itemsUpdated = 0;
    ...
    const updatedData = data.map(item => {
      ...
      if (codeFormatMode === 'multitrade') {
        ...
        if (liveActivity !== undefined) {
          ...
          const hasLevelOverride = shouldUseLevelActivity(costHead, bldgIdStd, effectiveOverrides);
          if (hasLevelOverride) {
  ...
  }, [data, mappings, itemTypeMappings, categoryMappings, materialDescOverrides, projectId, batchUpdateAppliedStatus, onDataUpdate, buildFullLaborCode]);
      ...
      {hasUnappliedChanges && (
        <div className="sticky top-0 z-50 flex items-center justify-between gap-4 rounded-lg border border-amber-300 bg-amber-50 px-6 py-3 text-amber-800">
          <div className="flex items-center gap-2 text-sm font-medium">
            <span>⚠️</span>
            <span>You have unapplied system mapping changes. Click "Apply All Mappings" before leaving this tab.</span>
          </div>
          <Button
            size="sm"
                                  ...
                                  >
                                    <Check
                                      className={cn(
                                        ...
                                        filteredSystems.find(s => selectedSystems.has(normalizeSystemKey(s.system)))?.laborCode === code.code
